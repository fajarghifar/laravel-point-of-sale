                    ...
                    <form action="{{ route('attendance.store') }}" method="POST">
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input id="datepicker" class="form-control @error('date') is-invalid @enderror" name="date" value="{{ old('date', $date) }}" autocomplete="off" />
                                @error('date')
                                                        ...
                                                        <div class="input-group justify-content-center">
                                                            ...
                                                            <div class="input-group-text bg-transparent border-0">
                                                                <div class="custom-control custom-radio custom-control-inline">
                                                                    <input type="radio" id="present{{ $key }}" name="status{{ $key }}" class="custom-control-input set-present" value="present" {{ $attendence->status == 'present' ? 'checked' : '' }}>
                                                                    <label class="custom-control-label text-success" for="present{{ $key }}">Present</label>
                                                                </div>
                                                            </div>
                                                            {{-- Status: Leave --}}
                                                            <div class="input-group-text bg-transparent border-0">
                                                                <div class="custom-control custom-radio custom-control-inline">
                                                                    <input type="radio" id="leave{{ $key }}" name="status{{ $key }}" class="custom-control-input set-leave" value="leave" {{ $attendence->status == 'leave' ? 'checked' : '' }}>
                                                                    <label class="custom-control-label text-warning" for="leave{{ $key }}">Leave</label>
                                                                </div>
                                                            </div>
                                                            {{-- Status: Absent --}}
                                                            <div class="input-group-text bg-transparent border-0">
                                                                <div class="custom-control custom-radio custom-control-inline">
                                                                    <input type="radio" id="absent{{ $key }}" name="status{{ $key }}" class="custom-control-input set-absent" value="absent" {{ $attendence->status == 'absent' ? 'checked' : '' }}>
                                                                    <label class="custom-control-label text-danger" for="absent{{ $key }}">Absent</label>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
